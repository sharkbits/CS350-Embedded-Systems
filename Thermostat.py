...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        ##
        if temp < self.setPoint and self.current_state.id == "heat":
            redLight.pulse()
        elif temp > self.setPoint and self.current_state.id == "cool":
            blueLight.pulse()
        elif temp >= self.setPoint and self.current_state.id == "heat":
            redLight.on()
        elif temp <= self.setPoint and self.current_state.id == "cool":
            blueLight.on()
    ...
    def setupSerialOutput(self):
        ...
        output = f"State: {self.current_state.id}, SetPoint: {self.setPoint}, Temp: {temp:.2f}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ##
            lcd_line_1 = f"{current_time.strftime('%b. %d %H:%M:%S')}\n"

            ## Setup Display Line 2
            if altCounter < 6:
                ##
                lcd_line_2 = "Cur:{temp:.2f}F".format(temp=self.getFahrenheit())
                altCounter = altCounter + 1
            else:
                ##
                lcd_line_2 = f"Set:{self.setPoint}F ({self.current_state_value})"
